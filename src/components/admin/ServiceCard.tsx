  ...
  return (
    <div className="service-card group relative bg-white border border-slate-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-all">
      ...
      <div className="hover-actions absolute inset-0 bg-white/95 rounded-xl flex items-center justify-center space-x-3 px-6">
        ...
        <button
          className={`p-2.5 border border-slate-200 rounded-lg transition-colors ${
            canLinkCategoryTranslation
              ? 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
              : 'text-slate-300 cursor-not-allowed bg-slate-50'
          }`}
          title={canLinkCategoryTranslation ? 'Link Translation' : 'Switch to English to link translations'}
          onClick={(e) => {
            e.stopPropagation();
            if (!canLinkCategoryTranslation) return;
            onLinkCategoryTranslation(id);
          }}
          disabled={!canLinkCategoryTranslation}
        >
